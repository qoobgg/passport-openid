 ...
 *
 * Options:
 *   - `callbackURL`         URL to which the OpenID provider will redirect the user after authentication
...
function Strategy(options, verify) {
  ...
  this._createRelyingParty = function(callbackURL) {
...
Strategy.prototype.authenticate = function(req, options) {
  var callbackURL = options.callbackURL || options.returnURL || this._callbackURL;
  if(!callbackURL) {
      return self.error(new Error('callbackURL must be provided either in the strategy options or to .authenticate'));
  }
  ...
  if (req.query && req.query['openid.mode']) {
    ...
    var self = this;
    this._createRelyingParty(callbackURL).verifyAssertion(req.url, function(err, result) {
    ...
    var self = this;
    this._createRelyingParty(callbackURL).authenticate(identifier, false, function(err, providerUrl) {
